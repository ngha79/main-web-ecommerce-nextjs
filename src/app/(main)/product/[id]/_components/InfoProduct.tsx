...
const InfoProduct = ({ product, shop }: { product: Product; shop: Shop }) => {
  ...
  return (
    <div className="bg-white shadow-login rounded-md w-full flex flex-col md:flex-row p-4">
      <div className="flex-1 h-max flex flex-col gap-4 items-center p-4">
        <div className="w-full max-w-full h-max flex items-center justify-center">
          <Image
            alt="image"
            src={
            ...
            width={450}
            height={450}
            className="w-auto max-w-full max-h-[300px] rounded-md md:max-h-[450px]"
          />
        </div>
        <CarouselThumb images={product.picture} />
        <LikeProduct productId={product.id} shopId={shop.shop_id} />
      </div>
